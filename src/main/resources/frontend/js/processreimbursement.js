...
$('#resolveRequestForm').submit(function(e){
    e.preventDefault();
	let formdata = $(this).serialize();
	$('#resolveRequest').modal('toggle');
	$.ajax({
		url: '/mi6/users/resolvereimursement',
		type: 'PATCH',
		data: formdata
	});
});

$('#resolveRequest').on('hidden.bs.modal', function () {
	document.getElementById('resolveRequestForm').reset();
});
...
function getReimbursements(){
	...
	xhttp.open('GET', '/mi6/users/processreimbursements');
		...
		for(let element in reimbursements[i]){
			...
			if(element == "Status"){
				...
				if(statusText == 'Pending'){
					addCell.innerHTML  = 
						`<button type="button" class="btn btn-warning" data-toggle='modal' data-target='#resolveRequest' 
						onClick="(function (){document.getElementById('requestID').setAttribute('value', '${statusRequestId}');})();">${statusText}</button>`;
